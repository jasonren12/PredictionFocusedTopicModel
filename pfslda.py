...
class pfsLDA(nn.Module):
    ...
    def __init__(self, K, V, M, M_val, p, alpha_fixed, device):
        ...
        # model parameters
        alpha = (
            torch.ones(self.K).to(device)
            if self.alpha_fixed
            else ds.Exponential(1).sample([self.K])
        )
    ...
    def ELBO(self, W_batch, phi_batch, gamma_batch, y_batch, version="real"):
        ...
        M = W_batch.shape[0]
        N_tot = W_batch.sum()
        ss = torch.digamma(gamma_batch) - torch.digamma(
            gamma_batch.sum(dim=1, keepdim=True)
        )
        ...
        second_term = contract("mkv,mk,mv->", phi, ss, W_batch, backend="torch")

        third_term1 = contract(
            "mkv,mv,kv,v->", phi, W_batch, beta.log(), varphi, backend="torch"
        )
        third_term2 = contract("mv,v,v->", W_batch, pi.log(), varphi, backend="torch")
        third_term3 = contract("mv,v->", W_batch, pi.log(), backend="torch")
        third_term = third_term1 - third_term2 + third_term3

        fourth_term1 = contract("mv,v->", W_batch, varphi, backend="torch")
        fourth_term2 = contract("mv,v->", W_batch, 1 - varphi, backend="torch")
        fourth_term = p.log() * fourth_term1 + (1 - p).log() * fourth_term2

        fifth_term = (
            torch.lgamma(gamma_batch.sum(dim=1)).sum()
            - torch.lgamma(gamma_batch).sum()
            + contract("mk,mk->", ss, gamma_batch - 1, backend="torch")
        )

        sixth_term = contract("mkv,mkv,mv->", phi, phi.log(), W_batch, backend="torch")
        ...
        seventh_term1 = contract(
            "mv,v,v->", W_batch, varphi, varphi.log(), backend="torch"
        )
        varphi = orig_varphi
        if varphi.max() >= 1:
            c = varphi.max().detach()
        ...
        seventh_term2 = contract(
            "mv,v,v->", W_batch, 1 - varphi, (1 - varphi).log(), backend="torch"
        )
    ...
    def theta_map(self, W, num_epochs=500, lr=0.005):
        theta_maps = torch.ones(
            (W.shape[0], self.K), requires_grad=True, device=self.device
        )
    ...
    def theta_post(self, W, theta):
        bl = contract(
            "kv,mk->mv", self.beta.softmax(dim=1), theta.softmax(dim=1), backend="torch"
        )

        out1 = contract(
            "v,mv,mv->m", self.varphi.sigmoid(), W, bl.log(), backend="torch"
        )
